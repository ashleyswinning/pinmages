...
from .helpers import (
    ...
    PointError, rect, get_height_and_width_for_max)
...
class Surface(object):
    ...
    def __init__(self, tree, output, dpi, parent_surface=None,
        ...
        width, height, viewbox = node_format(self, tree, parent_width, parent_height)
        # Actual surface dimensions: may be rounded on raster surfaces types
        scaled_width, scaled_height = get_height_and_width_for_max(width, height, parent_width, parent_height)
        self.cairo, self.width, self.height = self._create_surface(
            scaled_width * self.device_units_per_user_units,
            scaled_height * self.device_units_per_user_units)
        self.context = cairo.Context(self.cairo)
        # We must scale the context as the surface size is using physical units
        self.context.scale(
            (scaled_height/float(height)) * self.device_units_per_user_units, (scaled_width/float(width)) * self.device_units_per_user_units)
